...
#    print sys.argv[1]
print "Usage:"
print "./makeplotstackq2p1cx.py <res> <no_scale|scaled>\n"
...
pre="../data/"
str3=pre+"plot_kxq2p1_%(res)s_%(jump)s_%(scale)s.txt" % {"res": res, "jump": "10e3", "scale": scale}
str6=pre+"plot_kxq2p1_%(res)s_%(jump)s_%(scale)s.txt" % {"res": res, "jump": "10e6", "scale": scale}
str9=pre+"plot_kxq2p1_%(res)s_%(jump)s_%(scale)s.txt" % {"res": res, "jump": "10e9", "scale": scale}
...
if float(pen3[0]) < 0.000001:
    pen3[0] = "0.0000001"   # Want to include 0 penalty on the graph
if float(pen6[0]) < 0.000001:
    pen6[0] = "0.0000001"   # Want to include 0 penalty on the graph
if float(pen9[0]) < 0.000001:
    pen9[0] = "0.0000001"   # Want to include 0 penalty on the graph
...
str="SolKx Condition Numbers: $ %(res)s^2  \, $ %(scale)s\n Q2-P1 elements" % {"res": res, "scale": scale}
plt.title(str)
...
str=d+"stacksolkxq2p1ConditionNumbers%(res)s%(scale)s" % {"res": res, "scale": scale}
plt.savefig(str)
